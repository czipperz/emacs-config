...
;;; Code:
(defun sh-script-extra-font-lock-match-var-in-command-substitution (limit)
  "Search for variables in backticks or dollar parenthesized (`$()') strings.
LIMIT is the place at which the search will end."
  (let (res)
    (while (and (setq res (progn (if (eq (get-byte) ?$) (backward-char))
                                 (re-search-forward
                                  (concat
                                   "[^\\\\]\\$\\({#?\\)?"
                                   "\\([[:alpha:]_][[:alnum:]_]*"
...
(defvar sh-script-extra-font-lock-keywords
  '((sh-script-extra-font-lock-match-var-in-command-substitution
